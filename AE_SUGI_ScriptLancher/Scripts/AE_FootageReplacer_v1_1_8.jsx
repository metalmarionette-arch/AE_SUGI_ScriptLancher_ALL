...
(function () {
    ...
    var cDebugLog  = { value: true }; // 詳細ログ保存は常に有効（UIチェックボックスは削除）
    ...
    function replaceAllUnderFolder(targetAEFolder, newRootFolder, useFolderKey, useNameKey, pathOpt){
        ...
        function _extOfName(n){ var s=(""+n).toLowerCase(); var dot=s.lastIndexOf("."); return (dot<0) ? "" : s.substring(dot+1); }
        // 連番として扱う拡張子（静止画系のみ。動画/音声は除外して誤判定を減らす）
        var _seqImgExt = {"jpg":1,"jpeg":1,"png":1,"tif":1,"tiff":1,"tga":1,"bmp":1,"gif":1,"exr":1,"dpx":1,"cin":1};
        function _isSeqFile(file){
            try{
                if (!(file instanceof File)) return false;
                var ext = _extOfName(file.name);
                if (!_seqImgExt[ext]) return false;
                return _digitsSeqName(file.name);
            }catch(e){ return false; }
        }
        function _isStillFootage(it){
            try{
                if (it && it.mainSource && typeof it.mainSource.isStill === "boolean") return it.mainSource.isStill;
            }catch(e){}
            return false;
        }
        function _isSequenceFootage(it){
            try{
                if (it && it.mainSource && typeof it.mainSource.isSequence === "boolean") return it.mainSource.isSequence;
            }catch(e){}
            // fallback: 画像系の拡張子＋末尾連番のみ（動画名の "shot_010.mov" 等を誤って連番扱いしないため）
            try{
                if (_isStillFootage(it)) return false;
                if (!it || !it.mainSource || !it.mainSource.file) return false;
                return _isSeqFile(it.mainSource.file);
            }catch(e){}
            return false;
        }
        function _replacePreserveType(it, cand){
            // 1) 元が静止画なら常に静止画置換（連番読み込み抑止）
            // 2) 元が連番なら、候補が連番っぽいときだけ replaceWithSequence
            // 3) 置換後の素材名は「新ファイル名」または「name_[start-end].ext」形式に揃える（フォルダ名化を上書き）

            // cand を File に正規化
            var candFile = null;
            try{ candFile = (cand instanceof File) ? cand : new File(cand); }catch(_e){}
            if (!candFile || !(candFile instanceof File) || !candFile.exists) return;

            var wasStill = _isStillFootage(it);
            var wasSeq   = _isSequenceFootage(it);
            ...
            function _seqInfoFromFile(f){
                // f.name が test_0120.png なら、同フォルダ内の test_****.png を走査して
                // 表示名 test_[0120-0149].png と、必要なら先頭ファイルを返す
                var info = { isSeq:false, name:(f ? f.name : ""), headFile:f };
                if (!(f instanceof File) || !f.exists) return info;

                var m = f.name.match(/^(.*?)(\d{3,})\.(\w+)$/i);
                if (!m) return info;

                var prefix = m[1];
                var digitsStr = m[2];
                var width = digitsStr.length;
                var ext = m[3];

                // 静止画系だけ連番扱い（誤判定防止）
                if (!_seqImgExt[(ext+"").toLowerCase()]) return info;

                var dir = f.parent;
                if (!dir || !(dir instanceof Folder) || !dir.exists) return info;

                // prefix を正規表現用にエスケープ
                var esc = prefix.replace(/([.*+?^${}()|[\]\/\\])/g, "\\$1");
                var pat = new RegExp("^" + esc + "(\\d{" + width + "})\\." + ext + "$", "i");

                var files = dir.getFiles();
                var minN = null, maxN = null;
                var head = null;

                for (var i=0; i<files.length; i++){
                    var ff = files[i];
                    if (!(ff instanceof File)) continue;
                    var mm = ff.name.match(pat);
                    if (!mm) continue;
                    var n = parseInt(mm[1], 10);
                    if (isNaN(n)) continue;

                    if (minN === null || n < minN){ minN = n; head = ff; }
                    if (maxN === null || n > maxN) maxN = n;
                }

                // 2枚以上見つかったら連番とみなす（1枚しかないなら通常ファイル名）
                if (minN === null || maxN === null || minN === maxN || !(head instanceof File)) return info;

                info.isSeq = true;
                info.headFile = head; // 先頭（最小番号）
                info.name = prefix + "[" + _padNum(minN, width) + "-" + _padNum(maxN, width) + "]." + ext;
                return info;
            }

            // 置換
            var newName = candFile.name;
            if (wasStill){
                // 元が静止画 → 連番化させない
                it.replace(candFile);
                newName = candFile.name;
            }else if (wasSeq && _isSeqFile(candFile)){
                // 元が連番 → 置換先も連番として扱う（表示名も連番形式へ）
                var seqInfo = _seqInfoFromFile(candFile);
                // AE側の癖で「最後の1枚」基準でも連番として読めることがあるが、
                // 安定のため、可能なら先頭ファイルで置換する
                var seqHead = (seqInfo && seqInfo.isSeq && seqInfo.headFile) ? seqInfo.headFile : candFile;
                it.replaceWithSequence(seqHead, true);

                if (seqInfo && seqInfo.isSeq){
                    newName = seqInfo.name;
                }else{
                    // 連番として確定できない場合はファイル名
                    newName = candFile.name;
                }
            ...
            // 置換後に AE が素材名をフォルダ名などに変える場合があるので上書き
            try{ if (newName) it.name = newName; }catch(_e2){}
        }
