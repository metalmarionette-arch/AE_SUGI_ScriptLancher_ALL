...
(function PivotAnchorNullUI(thisObj) {
    ...
    // ---------------- Shared Null Footage ("ヌル"素材) ----------------
    // 既にプロジェクト内に「ヌル」を含むソリッド素材があればそれを使い、
    // 無ければ addNull / addSolid で生成されたソリッドを「ヌル_素材」として登録して共有します。
    function findSharedNullFootageItem() {
        try {
            ...
            for (var i = 1; i <= app.project.numItems; i++) {
                ...
                if (nm.indexOf("ヌル") === -1) continue;

                try {
                    if (it.mainSource && (it.mainSource instanceof SolidSource)) return it;
                } catch (e1) {}
            }
        } catch (e2) {}
        return null;
    ...
    function ensureFootageDurationAtLeast(footage, dur) {
        try {
            if (!footage) return;
            if (isNumber(dur) && isNumber(footage.duration) && footage.duration < dur) {
                footage.duration = dur;
            }
        } catch (e) {}
    }

    function applySharedNullSourceToLayer(layer, comp) {
        if (!layer) return null;

        var shared = findSharedNullFootageItem();
        var created = null;
        try { created = layer.source; } catch (e0) { created = null; }

        // 共有素材が無い場合：今回作ったソリッドを共有素材として採用
        if (!shared) {
            try {
                if (created && created.mainSource && (created.mainSource instanceof SolidSource)) {
                    created.name = "ヌル_素材";
                    shared = created;
                }
            } catch (e1) {}
        ...
        // 共有素材がある場合：それに差し替え（作成時のソリッドは不要なら削除）
        ensureFootageDurationAtLeast(shared, comp ? comp.duration : 0);

        try {
            if (created && shared && created !== shared) {
                layer.replaceSource(shared, false);

                // 作成時のソリッドが未使用なら削除（プロジェクトを汚さない）
                try {
                    if (created.usedIn && created.usedIn.length === 0) created.remove();
                } catch (e2) {}
            }
        } catch (e3) {}
        ...
        try {
            ...
            try {
                // addNull が無い / 失敗した場合はソリッドで代用（Point Control 用の器なのでOK）
        ...
        // 共有「ヌル」素材を使う（無ければ今回生成したソリッドを共有素材として登録）
        try { applySharedNullSourceToLayer(h, comp); } catch (e3) {}

        try { h.name = "__PAN_EVAL__"; } catch (e4) {}
        try { h.guideLayer = true; } catch (e5) {}
        try { h.shy = true; } catch (e6) {}
        try { h.enabled = true; } catch (e7) {}
        try {
        ...
        // Point Control (2D)
        var fx = null;
        try { fx = h.property("ADBE Effect Parade"); } catch (e9) { fx = null; }
        var ptProp = null;
        try {
            var pe = fx.addProperty("ADBE Point Control");
            if (pe) {
                pe.name = "__PAN_PT__";
                ptProp = pe.property("ADBE Point Control-0001");
            }
        } catch (e10) {
            ptProp = null;
        }
        if (!ptProp) {
            try { h.remove(); } catch (e11) {}
            return null;
        }

        try { h.locked = true; } catch (e12) {}
    ...
    function evalExprPoint2D(comp, expr, t) {
        ...
        var expr =
            ...
            "var p=L.toComp([" + x + "," + y + ",0]);\n" +
            "[p[0],p[1]];";
        var r = evalExprPoint2D(comp, expr, t);
        if (r) return r;

        // index がズレた等で失敗した場合：layer名で再試行（より安定）
        try {
            var nm = layer.name;
            nm = nm.split('\\').join('\\\\');
            nm = nm.split('"').join('\\"');
            var expr2 =
                ...
                "var p=L.toComp([" + x + "," + y + ",0]);\n" +
                "[p[0],p[1]];";
            return evalExprPoint2D(comp, expr2, t);
        } catch (e1) {}
...
// ★差し替え①：アンカー点の comp 座標を正しく取る（重要）
    ...
    // ---------------- Null creation helpers ----------------
        ...
        try {
            ...
            try {
                // addNull が無い / 失敗した場合はソリッドで代用（最悪でも親として使える）
                nl = comp.layers.addSolid([1,1,1], nameBase, 100, 100, comp.pixelAspect, comp.duration);
            } catch (e2) {
                return { ok: false, reason: "Null作成に失敗しました: " + e2.toString(), layer: null };
            }
        }

        // 共有「ヌル」素材を使う（無ければ今回生成したソリッドを共有素材として登録）
        try { applySharedNullSourceToLayer(nl, comp); } catch (e3) {}

        try { nl.name = nameBase; } catch (e4) {}
        // ★要望：配置したヌルはガイドレイヤーにする
        try { nl.guideLayer = true; } catch (e5) {}
        try { nl.threeDLayer = false; } catch (e6) {}

        // ★ヌルのアンカーは(0,0)に揃える（ピボット合わせがわかりやすい）
        try {
            var ap = nl.property("ADBE Transform Group").property("ADBE Anchor Point");
            if (ap && ap.canSetExpression === false) {
                // ok
            }
            if (ap && ap.canVaryOverTime) {
                ap.setValue([0,0]);
            } else if (ap) {
                ap.setValue([0,0]);
            }
    ...
    function copyRotScaleFromParent(parentLayer, targetLayer, tNow) {
        // 既存親がある時の「回転/スケールをヌルへコピー」用
        //
        // 注意：
        // targetLayer を parentLayer の子にした後に同値をコピーすると「二重掛け」になってしまいます。
        // そのため、すでに親子関係ができている場合は targetLayer 側は Identity
        // （Rotation=0 / Scale=100）へ戻し、親の回転/スケールは「継承」で揃える挙動にします。
        try {
            var sT = getScaleProp(targetLayer);
            var rT = getRotationZProp(targetLayer);

            var alreadyChild = false;
            try { alreadyChild = (targetLayer.parent === parentLayer); } catch (e0) { alreadyChild = false; }

            if (alreadyChild) {
                if (sT && isEditable(sT)) {
                    var tv = null;
                    try { tv = sT.value; } catch (e1) { tv = null; }

                    if (tv instanceof Array) {
                        var out = [];
                        for (var i = 0; i < tv.length; i++) out.push(100);
                        if (out.length < 2) out = [100, 100];
                        sT.setValue(out);
                    } else {
                        sT.setValue([100, 100]);
                    }
            ...
            // まだ親子関係が無い場合（フォールバック）：親の値をそのままコピー
    ...
    function runParentToPivotNull(comp, layers, pivotKey, optCopyParentRS) {
        ...
        for (var g = 0; g < orderKeys.length; g++) {
            ...
            if (parentLayer) {
                ...
                // ★pivotComp（Comp座標）を parent 空間へ変換して Position 設定（必ずピボット位置へ）
                var posInParent = calcPosInParentSpace(parentLayer, comp, tNow, pivotComp);

                if (!posInParent) {
                    // フォールバック：親アンカー(comp)との差分（回転/スケール無し想定）
                    var pA = getAnchorCompPos2D(parentLayer, comp, tNow);
                    if (pA) posInParent = [pivotComp[0] - pA[0], pivotComp[1] - pA[1]];
                }

                if (posInParent) {
                    if (!setPosition2D(nullLayer, posInParent)) {
                        ng.push("[" + safeName(nullLayer) + "] null_pos_set_fail(parent)");
                    }
                } else {
                    // どうしても変換できない場合は親のアンカー位置(0,0)に置く
                    setPosition2D(nullLayer, [0,0]);
                    ng.push("[" + safeName(nullLayer) + "] null_pos_calc_fail(parent)");
                }
