...
(function (thisObj) {
    ...
    function setCompDurationAndWorkArea(comp, durationSec) {
        ...
        // 1) 秒を「フレーム数」に変換してスナップ（丸め誤差対策）
        //    ※ 1フレーム未満にならないように最低1F
        var frames = Math.max(1, Math.round(durationSec / fd));
        var d = Math.max(fd, frames * fd);

        // 2) まずコンポ尺を確定
        comp.duration = d;

        // 3) ワークエリアを0開始にして、DurationはAE内部の正確な comp.duration を使う
        comp.workAreaStart = 0;

        // まれに境界でコケるAEがあるので、念のため例外時はほんの僅かだけ短くして再設定
        try {
            comp.workAreaDuration = comp.duration;
        } catch (e) {
            var eps = fd / 1000; // 1/1000フレーム程度（見た目に影響しない）
            comp.workAreaDuration = Math.max(fd, comp.duration - eps);
        }
    }
