...
(function AnimPropSelect(thisObj) {
    ...
    // ---------- 両方OFF（キー付き/式付き）時の「非アニメでも対象にする」収集 ----------
    function isDirectNoAnimCategory(cat) {
        // 「ボタン1つ＝特定プロパティ（またはその次元分割）」として扱えるものだけ
        // ※Effect/Mask/Shape/Text/Other は対象が広すぎるため除外
        return (
            cat === "Position" || cat === "PositionX" || cat === "PositionY" || cat === "PositionZ" ||
            cat === "Rotation" || cat === "RotationX" || cat === "RotationY" || cat === "RotationZ" ||
            cat === "Scale" ||
            cat === "AnchorPoint" ||
            cat === "Opacity" ||
            cat === "Orientation" ||
    ...
    function collectLeafPropsFromContainer(containerProp) {
        // containerProp が leaf(PROPERTY) ならそれを返し、Group なら直下の leaf を返す
        var out = [];
        if (!containerProp) return out;

        var pt = null;
        try { pt = containerProp.propertyType; } catch (e0) { pt = null; }

        if (pt === PropertyType.PROPERTY) {
            out.push(containerProp);
            return out;
        }

        var n = 0;
        try { n = containerProp.numProperties; } catch (e1) { n = 0; }
        for (var i = 1; i <= n; i++) {
            var p = null;
            try { p = containerProp.property(i); } catch (e2) { p = null; }
            if (!p) continue;

            var cpt = null;
            try { cpt = p.propertyType; } catch (e3) { cpt = null; }
            if (cpt === PropertyType.PROPERTY) {
                out.push(p);
            }
        }
        return out;
    ...
    function addUniqueLeafProps(leafProps, outArr, seenMap) {
        if (!leafProps || leafProps.length === 0) return;
        for (var i = 0; i < leafProps.length; i++) {
            var p = leafProps[i];
            if (!p) continue;
            var key = "";
            try { key = propPathToString(getPropIndexPath(p)); } catch (e0) { key = ""; }
            if (!key) continue;
    ...
    function addLeafsFromMatchName(container, matchName, outArr, seenMap) {
        if (!container) return;
        var p = null;
        try { p = container.property(matchName); } catch (e0) { p = null; }
        if (!p) return;
        addUniqueLeafProps(collectLeafPropsFromContainer(p), outArr, seenMap);
    }

    function findAllLeafPropsByMatchName(group, matchName, outArr, seenMap) {
        if (!group) return;
        var n = 0;
        try { n = group.numProperties; } catch (e0) { n = 0; }
        for (var i = 1; i <= n; i++) {
            var p = null;
            try { p = group.property(i); } catch (e1) { p = null; }
            if (!p) continue;

            var pt = null;
            try { pt = p.propertyType; } catch (e2) { pt = null; }

            if (pt === PropertyType.PROPERTY) {
                var mn = "";
                try { mn = p.matchName; } catch (e3) { mn = ""; }
                if (mn === matchName) {
                    addUniqueLeafProps([p], outArr, seenMap);
                }
            }
            ...
            try { subN = p.numProperties; } catch (e4) { subN = 0; }
            if (subN && subN > 0) {
                findAllLeafPropsByMatchName(p, matchName, outArr, seenMap);
            }
        }
    }

    function collectPropsForCategoryNoAnim(layer, cat) {
        // キー/式が無くても「カテゴリに紐づくプロパティ」を返す
        var out = [];
        if (!layer) return out;
        ...
        function addTransform(matchName) {
            if (!t) return;
            addLeafsFromMatchName(t, matchName, out, seen);
        }

        function addPosAll() {
            addTransform("ADBE Position");
            addTransform("ADBE Position_0");
            addTransform("ADBE Position_1");
            addTransform("ADBE Position_2");
        }
        function addAnchorAll() {
            addTransform("ADBE Anchor Point");
            addTransform("ADBE Anchor Point_0");
            addTransform("ADBE Anchor Point_1");
            addTransform("ADBE Anchor Point_2");
        }
        function addScaleAll() {
            addTransform("ADBE Scale");
            addTransform("ADBE Scale_0");
            addTransform("ADBE Scale_1");
            addTransform("ADBE Scale_2");
        }
        function addRotationAll() {
            addTransform("ADBE Rotate Z");
            addTransform("ADBE Rotate X");
            addTransform("ADBE Rotate Y");
        }
        ...
        if (cat === "All") {
            ...
            addScaleAll();
            addAnchorAll();
            addTransform("ADBE Opacity");
            addTransform("ADBE Orientation");
            addTransform("ADBE Point of Interest");
        } else if (cat === "Position") {
            addPosAll();
        } else if (cat === "PositionX") {
            addTransform("ADBE Position_0");
        } else if (cat === "PositionY") {
            addTransform("ADBE Position_1");
        } else if (cat === "PositionZ") {
            addTransform("ADBE Position_2");
        } else if (cat === "Rotation") {
            addRotationAll();
        } else if (cat === "RotationX") {
            addTransform("ADBE Rotate X");
        } else if (cat === "RotationY") {
            addTransform("ADBE Rotate Y");
        } else if (cat === "RotationZ") {
            addTransform("ADBE Rotate Z");
        } else if (cat === "Scale") {
            addScaleAll();
        } else if (cat === "AnchorPoint") {
            addAnchorAll();
        } else if (cat === "Opacity") {
            addTransform("ADBE Opacity");
        } else if (cat === "Orientation") {
            addTransform("ADBE Orientation");
        } else if (cat === "PointOfInterest") {
            addTransform("ADBE Point of Interest");
        }

        // Audio Levels（Transform外のことがあるので再帰で拾う）
        if (cat === "AudioLevels" || cat === "All") {
            findAllLeafPropsByMatchName(layer, "ADBE Audio Levels", out, seen);
        }
    ...
    function selectByCategory(cat) {
        // comp/layer は先に確定（scan しない分岐があるため）
        state.comp = getActiveComp();
        state.layers = getSelectedLayers(state.comp);
        state.items = [];
        ...
        // ★両方OFF（キー付き/式付き）時は、Transform系ボタンに限り
        // アニメの有無に関係なく「カテゴリのプロパティ」を対象にする
        var useNoAnimTargets = (!state.includeKeys && !state.includeExpr && (state.doSelectProps || state.addKeyAtCursor) && isDirectNoAnimCategory(cat));
        ...
        // 実行に使う items（scan結果 or 非アニメ収集）
        var workItems = [];
        if (useNoAnimTargets) {
            for (var l = 0; l < state.layers.length; l++) {
                var ly = state.layers[l];
                var props = collectPropsForCategoryNoAnim(ly, cat);
